...
$sql='SELECT id,link,link_name,menu,link_turn,title,meta_d,meta_k,caption,
	img_s,img_alt_s,img_title_s,short_text
	FROM '.$table_name.' ORDER BY menu,link_turn  LIMIT '.$start.','.$msg;
	...
	if($res['img_s']!=''){$img_s='<a href="/'.$uri_parts[0].'/'.$res['link'].'"><img class="fl five img_link" src="/img/'.$img_dir.'/dbpic.php?id='.$res['img_s'].'" alt="'.$res['img_alt_s'].'" title="'.$res['img_title_s'].' - узнать подробнее..."></a>';}else{$img_s='';}

if($res['menu']==1&&$step!=$res['menu']){$search_content.='<div class="cl"><h4>С чего начать?</h4></div>';$step=$res['menu'];}
if($res['menu']==2&&$step!=$res['menu']){$search_content.='<div class="cl"><h4>Чем ловить?</h4></div>';$step=$res['menu'];}
if($res['menu']==3&&$step!=$res['menu']){$search_content.='<div class="cl"><h4>На что ловить?</h4></div>';$step=$res['menu'];}
...
//***конец
}}mysql_free_result($result);$keywords.='школа рыболова';?>